  ...
  head: () => ({
    meta: [
      { title: "Tabela de Preços · Tatiane Penteado" },
      {
        name: "description",
        content:
          "Tabela de preços de massagens, tratamentos faciais e packs do Espaço Tatiane Penteado.",
      },
    ],
  }),
...
const integrativeTherapies = [
  {
    label: "Consulta de Medicina Tradicional Chinesa MTC",
    value: "Sob Consulta",
  },
...
function PriceRow({
  ...
  return (
    <div
      id={id}
      className="group flex items-baseline gap-4 border-b border-dashed border-border py-4 scroll-mt-32 transition-colors target:bg-gold-soft/10 target:px-4 target:-mx-4 target:border-gold/50 rounded-sm"
    >
      <span className="text-sm font-medium text-foreground md:text-base">
        {label}
      </span>
      <span
        className="flex-1 border-b border-dotted border-border/60"
        aria-hidden
      />
    ...
    <Layout>
      <section className="container-narrow py-20 md:py-28">
        <SectionHeading
          ...
          title={
            <>
              Investimento no seu <em className="text-gold">bem-estar</em>
            </>
          }
        ...
        <div className="mx-auto mt-20 max-w-4xl">
          ...
          <h3 className="text-center font-serif text-2xl text-primary">
            <span className="gold-divider align-middle" />{" "}
            <span className="px-4 font-medium uppercase tracking-[0.3em] text-sm text-gold">
              Tratamentos de Assinatura
            </span>{" "}
            <span className="gold-divider align-middle" />
          </h3>
          <div className="mt-10 mx-auto max-w-3xl">
            {mainTreatments.map((t) => (
              <PriceRow key={t.id} id={t.id} label={t.label} value={t.value} />
            ))}
          </div>
          ...
          <div className="mt-10 mx-auto max-w-3xl">
            {massages.map((m) => (
              <PriceRow key={m.label} label={m.label} value={m.value} />
            ))}
          </div>
          ...
          <div className="mt-10 mx-auto max-w-3xl">
            {integrativeTherapies.map((it) => (
              <PriceRow
                ...
                label={t(it.label)}
                value={
                  it.value === "Sob Consulta" ? t("Sob Consulta") : it.value
                }
              />
            ))}
          </div>
          ...
          <p className="mt-4 text-center text-muted-foreground text-sm max-w-2xl mx-auto">
            Ao optar por um programa contínuo, não só poupa no valor da sessão,
            como garante resultados muito mais expressivos e duradouros.
          </p>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {packs.map((p) => (
              <div
                key={p.id}
                id={p.id}
                className="flex flex-col border border-border/60 bg-cream p-8 text-center transition-all hover:border-gold/50 scroll-mt-32 target:border-gold target:shadow-sm"
              >
          ...
          <div className="mt-32 bg-background border border-border p-8 md:p-12">
            <h3 className="font-serif text-2xl text-primary mb-8 border-b border-border/50 pb-4">
            ...
            <div className="grid gap-8 sm:grid-cols-2">
              <div>
                ...
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
                  Aceitamos pagamentos em numerário, MBWay e transferência
                  bancária. O IVA está incluído em todos os preços apresentados
              ...
              <div>
                ...
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
                  O pagamento dos packs pode ser dividido, sendo 50% pago na 1ª
                  sessão e o restante na 2ª sessão. Os packs têm validade de 6
                  meses após a compra.
                </p>
              </div>
              <div>
                ...
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
                  Pedimos gentilmente que qualquer alteração ou cancelamento
                  seja comunicado com 24 horas de antecedência para
              ...
              <div>
                ...
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
                  Caso tenha um objetivo específico não listado aqui, fale
                  connosco. Desenhamos protocolos personalizados e à medida das
                  suas necessidades.
                </p>
              </div>
            </div>
